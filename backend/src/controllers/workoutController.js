...
// ✅ GOAL UPDATE & RESET LOGIC
const goal = await prisma.goal.findFirst({ where: { userId: Number(req.user.id) } });

if (goal) {
    const newValue = goal.currentValue + Number(duration);
    ...
    if (newValue >= goal.targetValue) {
        const { saveGoalToHistoryAndReset } = require('./userController');
        await saveGoalToHistoryAndReset(req.user.id, goal.id);
    }
}
...
const updateWorkout = async (req, res) => {
    try {
        ...
        const updatedWorkout = await Workout.findByIdAndUpdate(
            req.params.id,
            req.body, // jo bhi naya data hai body me aaya hai
            { new: true, runValidators: true } // new: true, means updated data wapas do 
        );
